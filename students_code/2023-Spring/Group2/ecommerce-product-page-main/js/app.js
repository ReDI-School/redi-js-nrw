(() => {
    ...
    function initSliders() {
        if (document.querySelector(".shop__slider")) new core(".shop__slider", {
            modules: [ freeMode, Navigation, Thumb, EffectFade ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            spaceBetween: 0,
            ...
            FreeMode: true,
            thumbs: {
                swiper: new core(".shop__slider-thumbs", {
                    modules: [ freeMode, Navigation, Thumb, EffectFade ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 4,
        ...
        if (document.querySelector(".shop_in-new-tab-swiper")) new core(".shop_in-new-tab-swiper", {
            modules: [ freeMode, Navigation, Thumb, EffectFade ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            FreeMode: true,
            thumbs: {
                swiper: new core(".shop_in-new-tab-swiper-thumbs", {
    ...
    menuIcon.onclick = () => {
        let menuOpen = document.querySelector(".header");
        menuOpen.classList.toggle("menu-open");
        document.querySelector("html").classList.toggle("lock");
    };
    const counObj = document.querySelector(".button-description__count");
    const BtnPlus = document.querySelector(".button-description__plus");
    const BtnMinus = document.querySelector(".button-description__minus");
    const basketCount = document.querySelector("[data-basket]");
    BtnPlus.onclick = () => {
        counObj.innerHTML++;
    };
    BtnMinus.onclick = () => {
        if (counObj.textContent > 0) counObj.innerHTML--;
    };
    const btnAdd = document.querySelector("[data-add]");
    btnAdd.onclick = () => {
        headerCount.innerHTML = +headerCount.innerHTML + +counObj.textContent;
        basketCount.innerHTML = +headerCount.textContent;
        console.log(counObj.textContent);
        addTotalPrice();
        checkItems();
    };
    const headerPrice = document.querySelector(".about-card__price").firstElementChild;
    const headerCount = document.querySelector(".about-card__coun").firstElementChild;
    const headerTotalPrice = document.querySelector(".about-card__total-price").firstElementChild;
    function countTotalPrice(a, b) {
        return a * b;
    }
    function addTotalPrice() {
        headerTotalPrice.innerHTML = countTotalPrice(parseInt(headerPrice.textContent), parseInt(headerCount.textContent));
    }
    const cardItem = document.querySelector(".card__Items");
    const cardItemNon = document.querySelector(".card__Items_non");
    function checkItems() {
        if (0 == headerCount.textContent) {
            cardItem.classList.remove("active");
            cardItemNon.classList.add("active");
            basketCount.classList.add("active");
        } else {
            cardItem.classList.add("active");
            cardItemNon.classList.remove("active");
            basketCount.classList.remove("active");
        }
    }
    checkItems();
    const btnCancelCard = document.querySelector(".card__cancel");
    ...
    basket.addEventListener("click", (function() {
        ...
        console.log(headerCard.classList.contains("active"));
        if (headerCard.classList.contains("active")) main.onclick = () => {
            headerCard.classList.remove("active");
        };
    }));
    const sliderBlock = document.querySelector(".shop__image-block");
    const canselSliderBtn = document.querySelector(".in-new-tab-cansel");
    sliderBlock.ondblclick = () => {
        const sliderFull = document.querySelector(".shop__in-new-tab");
        sliderFull.classList.add("active");
        document.querySelector("html").classList.add("lock");
        canselSliderBtn.onclick = () => {
            sliderFull.classList.remove("active");
